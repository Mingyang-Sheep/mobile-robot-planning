...
| `base_frame` | `base_footprint` | 坐标系 | 底盘 坐标系 |
| `odom_frame` | `odom` | 坐标系 | 里程计 坐标系 |
| `map_frame` | `map` | 坐标系 | 地图 坐标系 |
...
这个 启动文件 使用 `teleop_twist_keyboard.py`，需要在可交互终端中运行。
...
已有地图导航看 [navigation.md](navigation.md)，地图/世界文件 配对看 [configuration_reference.md](configuration_reference.md)。
