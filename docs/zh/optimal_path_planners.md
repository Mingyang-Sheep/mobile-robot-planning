...
| 算法 | 参数 key | C++ 调试节点 | Python 调试节点 | 接入 move_base | 调试路径 | 使用代价地图 | 当前限制 |
|---|---|---|---|---|---|---|---|
| A* | `astar` | ✅ | ✅ | ✅ | ✅ | move_base 适配器使用代价地图 | 栅格搜索，路径可能贴近膨胀区，受 `costmap_cost_weight` 影响 |
| Dijkstra | `dijkstra` | ✅ | ✅ | ✅ | ✅ | move_base 适配器使用代价地图 | 栅格搜索，通常比 A* 更保守但开销更大 |
| D* | `dstar` | ✅ | ✅ | ✅ | ✅ | move_base 适配器使用代价地图 | 当前适配器标注为静态 D* 风格，并非完整动态重规划实现 |
| D* Lite | `dstar_lite` | ✅ | ✅ | ✅ | ✅ | move_base 适配器使用代价地图 | 适配器中有状态化 D* Lite 接入，但仍需在真实动态障碍场景中继续评估 |
| Theta* | `theta_star` | ✅ | ✅ | ✅ | ✅ | move_base 适配器使用代价地图 | 使用视线检测，结果依赖碰撞检测和地图分辨率 |
| RRT* | `rrt_star` | ✅ | ✅ | ✅ | ✅ | move_base 适配器使用代价地图 | 采样算法，结果和随机采样、迭代上限有关 |
| Cubic Spline | `cubic_spline` | ✅ | ✅ | 🟣 | ✅ | 适配器平滑后做碰撞检查 | 不是独立全局规划器，不能写成 `global_planner` |
| DWA 调试 | `dwa` | ✅ | ✅ | ✅ | ✅ | 独立调试节点读取地图/里程计 | 独立 DWA 调试节点不等同于 move_base 内部 DWA |
...
- move_base 适配器 发布 `/mr_traditional_planner/executed_global_path`。
...
- `D*` 当前应视为静态代价地图 上的 D* 风格搜索适配，不要描述成完整动态 D* 系统。
- `D* Lite` 已有 move_base 适配器 和调试节点，但动态障碍持续重规划效果仍需实验数据支持。
- `RRT*` 采样结果可能随参数和随机性变化。
- Debug 节点多以 `/map` 和 RViz 目标 为输入，和 move_base 代价地图中的起点、代价、膨胀处理不一定完全相同。
- `Cubic Spline` 是平滑器 或调试节点，不是普通全局规划器。

## 7. 下一步阅读

路径含义看 [planner_framework.md](planner_framework.md)，话题 检查看 [topics_and_tf.md](topics_and_tf.md)。
