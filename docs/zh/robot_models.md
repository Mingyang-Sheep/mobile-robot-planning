...
适合读者：想在同一套 Gazebo / SLAM / 导航 链路中切换机器人模型的用户。
...
| 模型 key | 来源 | 主要文件 | base 坐标系 | 激光 坐标系 | 导航参数 key |
|---|---|---|---|---|---|
...
用途：默认 快速上手 模型，体积小，虚拟机上最稳。

主要特性：

- 差速底盘；
- 2D 激光 `/scan`，坐标系 为 `base_scan`；
- 里程计 `/odom`；
- 速度输入 `/cmd_vel`；
- IMU 话题 `/imu`；
...
- 差速底盘；
- 2D 激光 `/scan`；
- `robot_models.yaml` 记录相机 话题 `/camera/rgb/image_raw`；
- 导航仍主要依赖 `/scan`。
...
- 使用 `libgazebo_ros_laser.so` 发布 `/scan`，坐标系 为 `laser`；
- 使用 depth camera 插件发布相机/深度相关 话题；
- 使用 IMU 插件发布 `/imu`；
- 导航 footprint 和 代价地图参数独立于 TurtleBot3。
...
- DWA 参数文件是否存在；
- 激光 坐标系 是否和 代价地图的 `sensor_frame` 一致；
- footprint 是否适合真实尺寸；
- 初始位姿是否落在地图自由空间。
