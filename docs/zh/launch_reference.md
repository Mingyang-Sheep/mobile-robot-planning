...
[英文版](../launch_reference.md)

</div>

# 启动文件与启动参数参考

适合读者：想复制启动命令、切换机器人/地图/规划算法，或想确认某个 启动文件 是否适合直接使用的用户。

本文只统计当前项目包 `src/mr_*` 下的 启动文件，不把 `refer/` 中的外部参考仓库计入当前可维护入口。当前项目共有 40 个 `.launch` 文件。它们已通过静态验证：
...
| 启动文件 | 主要用途 | 默认机器人 | 默认地图/World | Gazebo | RViz | 推荐用户 |
|---|---|---|---|---|---|---|
| `mr_navigation/simulation.launch` | 只启动 Gazebo + 机器人，用于传感器和底盘仿真检查 | `burger` | `empty.world` | ✅ | ❌ | 初学者、模型调试 |
| `mr_navigation/navigation_sim.launch` | Gazebo + map_server + AMCL + move_base + RViz | `burger` | `turtlebot3_world` | ✅ | ✅ | 新手首选 |
| `mr_navigation/slam_sim.launch` | 从 navigation 包转发到 SLAM 仿真入口 | `burger` | `stage_1.world` | ✅ | ✅ | 需要从 navigation 包统一启动 SLAM 的用户 |
| `mr_slam/slam_sim.launch` | Gazebo + SLAM + RViz | `burger` | `stage_1.world` | ✅ | ✅ | 建图实验 |
| `mr_traditional_planner/planner_sim.launch` | navigation_sim + 独立传统算法调试节点 | `burger` | `turtlebot3_world` | ✅ | ✅ | 算法对比实验 |
| `mr_learning/dqn_train.launch` | Gazebo 阶段 1 + DQN 训练脚本 | `burger` | `stage_1.world` | ✅ | ❌ | 学习实验开发者 |
| `mr_description/wpb_home_description.launch` | 只显示 WPB Home / WPB Home Mani 描述模型 | `wpb_home` | 无 | ❌ | ✅ | URDF 检查 |
...
| `world_name` | `$(find mr_gazebo)/worlds/turtlebot3_world.world` | 世界文件路径 | Gazebo 世界文件 |
| `x` | `-2.0` | 数值 | 初始 x |
| `y` | `-0.5` | 数值 | 初始 y |
| `z` | `0.01` | 数值 | 初始 z |
| `yaw` | `0.0` | 数值 | 初始 yaw |
...
| `scan_topic` | `/scan` | 话题 | 激光 话题 |
| `odom_topic` | `/odom` | 话题 | 里程计 话题 |
| `cmd_vel_topic` | `/cmd_vel` | 话题 | 速度命令 话题 |
...
| `costmap_cost_weight` | `2.0` | 数值 | 自定义全局规划适配器的 代价地图软代价权重 |
...
用途：只启动仿真 世界文件 和机器人，不启动 map_server、AMCL、move_base、RViz。

| 参数 | 默认值 | 可选值 | 含义 |
|---|---|---|---|
| `model` | `burger` | 已支持机器人 key | 导航/底盘参数 key |
| `robot_model` | `$(arg model)` | 已支持机器人 key | URDF/Gazebo 模型 |
| `world_name` | `$(find mr_gazebo)/worlds/empty.world` | 世界文件路径 | Gazebo 世界文件 |
...
| `robot_model` | `$(arg model)` | 已支持机器人 key | URDF/Gazebo 模型 |
| `stage` | `1` | `1` 到 `4` | 默认 stage 世界文件 编号 |
| `world_name` | `$(find mr_gazebo)/worlds/stage_$(arg stage).world` | 世界文件路径 | Gazebo 世界文件 |
| `x`, `y`, `z`, `yaw` | `0`, `0`, `0.01`, `0` | 数值 | 初始位姿 |
| `gui` | `true` | `true`, `false` | Gazebo GUI |
| `headless` | `false` | `true`, `false` | 无头模式 |
| `use_rviz` | `true` | `true`, `false` | 是否启动 RViz |
...
用途：从 `mr_navigation` 包提供的 SLAM 仿真转发入口。实际 包含 `mr_slam/slam_sim.launch`。
...
用途：启动完整 导航 仿真，并额外启动一个独立 C++/Python 传统算法调试节点。
...
| `debug_path_topic` | 根据 algorithm 推导 | 话题 | 调试节点输出路径 |
...
这条命令中，`global_planner:=dstar_lite` 决定 move_base 实际使用 D* Lite 适配器；`algorithm:=dstar_lite impl:=cpp` 决定额外 C++ 调试节点发布 `/mr_traditional_planner/debug_optimal_path`。
...
| `path_topic` | `/mr_traditional_planner/debug_optimal_path` | 话题 | 输出路径 话题 |
...
用途：启动 Gazebo 阶段环境并运行 DQN 训练脚本。当前是实验性 演示，不是成熟强化学习平台。
...
| 启动文件 | 所属 包 | 用途 | 新手直接使用 |
|---|---|---|---|
| `mr_description/load_robot_description.launch` | `mr_description` | 按 `robot_model` 加载 `robot_description` | 否，通常被 包含 |
| `mr_description/wpb_home_description.launch` | `mr_description` | WPB Home / WPB Home Mani 描述模型检查 | 是，URDF 调试时可用 |
| `mr_gazebo/spawn_navigation_world.launch` | `mr_gazebo` | 导航场景 生成 世界文件 + robot | 否，被 `navigation_sim` 包含 |
| `mr_gazebo/spawn_robot.launch` | `mr_gazebo` | 通用 Gazebo 世界文件 + 机器人生成 | 否，被 simulation/SLAM/学习 包含 |
...
| `mr_navigation/amcl.launch` | `mr_navigation` | 启动 AMCL | 否，通常被 包含 |
| `mr_navigation/move_base.launch` | `mr_navigation` | 加载 代价地图、planner、DWA 并启动 move_base | 否，通常被 包含 |
| `mr_navigation/teleop_keyboard.launch` | `mr_navigation` | 键盘控制 `/cmd_vel` | 可用，但需要终端交互 |
| `mr_slam/slam.launch` | `mr_slam` | 按 `slam_method` 包含 SLAM 后端 | 已有机器人和传感器时可用 |
...
| Gazebo 生成 | 2 | `spawn_navigation_world.launch`, `spawn_robot.launch` |
| 地图 | 1 | `map_loader.launch` |
| 导航 | 7 | `simulation.launch`, `navigation_sim.launch`, `navigation.launch`, `move_base.launch`, `amcl.launch`, `slam_sim.launch`, `teleop_keyboard.launch` |
| SLAM | 4 | `slam.launch`, `slam_sim.launch`, `gmapping.launch`, `hector.launch` |
| Traditional planner | 22 | `planner.launch`, `planner_sim.launch` 和 20 个单算法 C++/Python 启动文件 |
| 学习 | 2 | `learning_env.launch`, `dqn_train.launch` |
...
| 错误组合 | 原因 | 正确写法 |
|---|---|---|
| `global_planner:=cubic_spline` | Cubic Spline 是平滑器 或调试节点，不是独立全局规划器 | `global_planner:=theta_star path_smoother:=cubic_spline` |
...
| `planning_mode:=coverage path_smoother:=cubic_spline` | 覆盖模式当前不使用 平滑器 | 去掉 `path_smoother` |
