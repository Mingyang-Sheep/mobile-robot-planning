...
`install_dependencies.sh` 会检查 Ubuntu 和 ROS 环境，并安装仓库中 package.xml、启动文件 和源码实际使用到的 ROS 包。脚本支持重复执行，已安装的 apt 包不会因此报错。
...
ROS 使用环境变量查找 包、消息类型、插件和动态库。常见变量包括：
...
- `pluginlib`
- `tf`、`tf2_ros`
- `slam_gmapping` 对应的 ROS 包 `gmapping`
- `hector_mapping`
- `rviz`
- `rqt_tf_tree`
- `teleop_twist_keyboard`

学习 模块使用 Python `numpy` 和 `torch`。`numpy` 可通过 apt 安装；`torch` 的版本通常受 CPU/GPU、CUDA 和 Python 环境影响，安装脚本默认只检查并提示，不强制安装 PyTorch。
