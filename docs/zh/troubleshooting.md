...
[英文版](../troubleshooting.md)

</div>

# 常见问题与排错

适合读者：启动文件 已经启动但没有 话题、机器人不动、RViz 报错或路径异常的用户。
...
通常等几秒即可。如果长时间不出现 `/gazebo` 相关 话题，按上一节排查 Gazebo 是否已经退出。

## 没有 `/scan`

**影响：** AMCL、SLAM、代价地图和规划器都会受影响。
...
- 导航 模式中先用 `2D Pose Estimate` 给 AMCL 初始位姿；
- 确认 AMCL 正在订阅 `/scan`；
- 确认 机器人底盘坐标系 是 `base_footprint`。
...
如果 `/odom` 没有变化，优先看 Gazebo 插件和机器人 生成。
...
**说明：** BCD/STC 中 RViz 点击只用于触发覆盖规划，不是覆盖终点。覆盖路径从机器人当前位置附近开始生成，并通过 `/move_base` 动作 逐点执行。
