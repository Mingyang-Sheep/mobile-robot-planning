...
| `/robot_state_publisher` | Gazebo 生成 启动文件 | 发布机器人内部 TF |
| `/gazebo` | Gazebo 启动文件 | 物理仿真、传感器和底盘插件 |
...
| `global_planner:=astar` | `mr_traditional_planner/GlobalPlannerAdapter` | 自定义 A* 适配器 |
| `global_planner:=dijkstra` | `mr_traditional_planner/GlobalPlannerAdapter` | 自定义 Dijkstra 适配器 |
| `global_planner:=dstar` | `mr_traditional_planner/GlobalPlannerAdapter` | 静态代价地图 上的 D* 风格实现 |
| `global_planner:=dstar_lite` | `mr_traditional_planner/GlobalPlannerAdapter` | D* Lite 适配器 |
| `global_planner:=theta_star` | `mr_traditional_planner/GlobalPlannerAdapter` | Theta* 适配器 |
| `global_planner:=rrt_star` | `mr_traditional_planner/GlobalPlannerAdapter` | RRT* 适配器 |
...
| `path_smoother:=cubic_spline` | 对全局路径做 Cubic Spline 平滑；如果碰撞检测失败，适配器 会回退原始路径 |
