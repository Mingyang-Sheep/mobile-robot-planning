...
| Topic | 来源 | 是否参与实际控制 | 用途 |
|---|---|---|---|
| `/mr_traditional_planner/executed_global_path` | `GlobalPlannerAdapter` | 是 | 显示自定义全局规划器实际返回给 move_base 的路径 |
| `/move_base/DWAPlannerROS/global_plan` | `DWAPlannerROS` | 是 | DWA 内部跟踪的全局参考 |
| `/move_base/DWAPlannerROS/local_plan` | `DWAPlannerROS` | 是 | DWA 当前局部轨迹 |
| `/mr_traditional_planner/debug_optimal_path` | 独立 C++/Python 算法节点 | 否，默认只显示 | 对比算法输出、检查路径质量 |
| `/mr_traditional_planner/coverage_path` | BCD/STC 覆盖节点 | 间接参与 | 覆盖路径和 waypoint 执行参考 |
...
在 `planner_sim.launch` 中，如果 `algorithm` 是普通全局规划算法且没有显式覆盖 `global_planner`，launch 会让 `global_planner` 默认跟随 `algorithm`。为了实验结果清晰，建议仍然显式写出：
