...
| 目录 | 内容 |
|---|---|
| `src/mr_navigation/config/` | 机器人模型注册、costmap、DWA、AMCL、move_base、planner 参数 |
...
|---|---|
| `xacro` | 模型对应的仿真 Xacro |
| `base_frame` | 底盘 frame，当前通常是 `base_footprint` |
| `laser_frame` | 激光 frame，TurtleBot3 为 `base_scan`，WPB Home 为 `laser` |
| `odom_frame` | 里程计 frame，当前为 `odom` |
| `scan_topic` | 激光 topic，当前统一为 `/scan` |
| `cmd_vel_topic` | 速度命令 topic，当前统一为 `/cmd_vel` |
| `odom_topic` | 里程计 topic，当前统一为 `/odom` |
...
| `global_frame` | global costmap 为 `map`，local costmap 为 `odom` |
...
机器人在墙角抖动时，通常看 `sim_time`、速度采样、costmap footprint 和 inflation。

## 5. AMCL 参数

通用参数主要在 launch 中设置，WPB Home 还有专用覆盖文件：
...
| `costmap_cost_weight` | 自定义全局规划 adapter 中 costmap 软代价权重 |
...
| `path_topic` | 调试路径输出 topic |

## 8. world/map 配对

当前已有静态地图配对：

| `map_name` | map 文件 | 推荐 world |
|---|---|---|
