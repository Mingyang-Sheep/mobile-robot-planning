...
| `turtlebot3_world.world` | Quick Start 和默认 Navigation 配套 world |
| `stage_1.world` 到 `stage_4.world` | Learning / SLAM stage 场景 |
| `maze/maze_1.world` 到 `maze/maze_6.world` | 迷宫场景 |

Navigation 需要静态地图配套。当前已有静态地图的组合是：
...
| RGB-D 相机 | `libgazebo_ros_depth_camera.so` | 发布相机和深度图像 topic |
...
| 现象 | 优先检查 |
|---|---|
| 没有 `/gazebo` | Gazebo 是否退出、world 是否能单独加载 |
| `spawn_model` 一直等待 | `/gazebo/spawn_urdf_model` 服务是否存在 |
| 没有 `/scan` | laser 插件、frame、机器人是否 spawn |
| 没有 `/odom` | diff drive 插件、wheel joint、Gazebo 是否运行 |
| `/cmd_vel` 无订阅者 | 底盘插件未加载或 topic remap 错误 |
