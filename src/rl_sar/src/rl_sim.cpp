...
RL_Sim::RL_Sim(int argc, char **argv)
{
    ...
    private_nh.param<std::string>("cmd_vel_topic", this->cmd_vel_topic, std::string("/cmd_vel"));
    private_nh.param<double>("cmd_vel_input_rate", this->cmd_vel_input_rate, 50.0);
    private_nh.param<bool>("auto_start", this->auto_start, false);
    private_nh.param<bool>("start_navigation", this->start_navigation, false);
    private_nh.param<double>("auto_locomotion_delay", this->auto_locomotion_delay, 4.0);
    this->auto_start_time = ros::Time::now();
    ...
    // subscriber
    this->cmd_vel_subscriber = nh.subscribe<geometry_msgs::Twist>(this->cmd_vel_topic, 10, &RL_Sim::CmdvelCallback, this);
...
void RL_Sim::RobotControl()
{
    ...
    if (this->auto_start)
    {
        const std::string state_name = this->fsm.current_state_ ? this->fsm.current_state_->GetStateName() : "";
        if (!this->auto_getup_requested && state_name == "RLFSMStatePassive")
        {
            this->control.SetKeyboard(Input::Keyboard::Num0);
            this->auto_getup_requested = true;
        }
        else if (state_name == "RLFSMStateGetUp" &&
                 (ros::Time::now() - this->auto_start_time).toSec() >= this->auto_locomotion_delay)
        {
...
{
    const auto it = std::find(msg->name.begin(), msg->name.end(), this->gazebo_model_name);
    if (it == msg->name.end())
    {
        return;
    }
...
)
{
    if (this->cmd_vel_input_rate > 0.0)
    {
        ...
        if (!this->last_cmd_vel_time.isZero() &&
            (now - this->last_cmd_vel_time).toSec() < 1.0 / this->cmd_vel_input_rate)
        {
            return;
        }
