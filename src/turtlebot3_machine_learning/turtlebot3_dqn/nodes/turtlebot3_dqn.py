...
# ================= 终极路径黑魔法开始 =================
current_dir = os.path.dirname(os.path.abspath(__file__))  # nodes 目录
parent_dir = os.path.dirname(current_dir)                 # turtlebot3_dqn 根目录
src_dir = os.path.join(parent_dir, 'src')                 # src 目录
inner_src_dir = os.path.join(src_dir, 'turtlebot3_dqn')   # src/turtlebot3_dqn 目录 (respawnGoal.py 的真正老家)

# 1. 屏蔽干扰项：踢掉所有 ROS 生成的替身文件夹 (devel/lib/...)，防止被李鬼遮挡
sys.path = [p for p in sys.path if 'devel/lib/turtlebot3_dqn' not in p]

# 2. 强行把真实的源码老家全部塞进最高优先级！
sys.path.insert(0, inner_src_dir)  # 最关键的一步！
sys.path.insert(0, src_dir)
sys.path.insert(0, current_dir)
sys.path.insert(0, parent_dir)
# ================= 终极路径黑魔法结束 =================

# from utils import log_utils  # 继续保持注释状态，阉割日志
