#include "mr_traditional_planner/coverage/bcd_planner.h"

#include <actionlib/client/simple_client_goal_state.h>
#include <geometry_msgs/PoseStamped.h>
#include <geometry_msgs/Quaternion.h>
#include <tf/transform_datatypes.h>

#include <algorithm>
#include <cmath>
#include <cstddef>
#include <limits>
#include <queue>
#include <unordered_set>
#include <utility>
#include <vector>

namespace mr_traditional_planner {
namespace coverage {

namespace {

struct OpenItem {
  double f;
  double h;
  int index;

  bool operator<(const OpenItem& other) const {
    if (f != other.f) {
      return f > other.f;
    }

    if (h != other.h) {
      return h > other.h;
    }

    return index > other.index;
  }
};

}  // namespace

BcdPlanner::BcdPlanner()
    : move_base_client_("/move_base", true),
      map_width_(0),
      map_height_(0),
      resolution_(0.0),
      origin_x_(0.0),
      origin_y_(0.0),
      robot_radius_(0.15),
      sweep_spacing_(0.2),
      goal_timeout_(30.0),
      is_executing_(false) {
  // 全覆盖算法统一以地图为输入。
  map_sub_ = nh_.subscribe("/map", 1, &BcdPlanner::mapCallback, this);
  // 与 A* 完全对齐：覆盖算法也统一使用 RViz 的 2D Nav Goal 作为触发入口。
  goal_sub_ = nh_.subscribe("/move_base_simple/goal", 1, &BcdPlanner::goalCallback, this);
  // 全覆盖路径统一输出到专用 Path 话题，便于可视化与评测。
  path_pub_ = nh_.advertise<nav_msgs::Path>("/mr_traditional_planner/coverage_path", 1, true);
}

void BcdPlanner::mapCallback(const nav_msgs::OccupancyGridConstPtr& msg) {
  latest_map_ = msg;
  map_width_ = static_cast<int>(msg->info.width);
  map_height_ = static_cast<int>(msg->info.height);
  resolution_ = msg->info.resolution;
  origin_x_ = msg->info.origin.position.x;
  origin_y_ = msg->info.origin.position.y;
  buildObstacleLookup();
}

void BcdPlanner::goalCallback(const geometry_msgs::PoseStampedConstPtr& msg) {
  if (!msg->header.frame_id.empty() && msg->header.frame_id != "map") {
    ROS_WARN_STREAM("BCD C++: 仅支持 map 坐标系触发，当前收到的是 "
                    << msg->header.frame_id << "。");
    return;
  }

  if (is_executing_) {
    ROS_WARN("BCD C++: 当前仍在执行覆盖任务，忽略重复触发。");
    return;
  }

  if (!latest_map_) {
    ROS_WARN("BCD C++: /map 尚未收到，无法开始覆盖规划。");
    return;
  }

  double start_world_x = 0.0;
  double start_world_y = 0.0;
  if (!lookupStartPose(start_world_x, start_world_y)) {
    return;
  }

  // 关键步骤：覆盖规划与 A* 完全一致，使用同一套世界坐标到栅格索引转换公式。
  const std::pair<int, int> start = worldToGrid(start_world_x, start_world_y);
  if (!inBounds(start.first, start.second)) {
    ROS_WARN("BCD C++: 清扫起点超出地图范围，停止执行。");
    return;
  }

  if (isObstacle(start.first, start.second)) {
    ROS_WARN("BCD C++: 清扫起点位于障碍物膨胀区内，停止执行。");
    return;
  }

  const std::vector<int> path_indices = buildCoveragePath(start.first, start.second);
  if (path_indices.empty()) {
    ROS_WARN("BCD C++: 未生成有效覆盖路径。");
    return;
  }

  publishPath(path_indices);

  is_executing_ = true;
  if (!waitForMoveBaseServer()) {
    is_executing_ = false;
    return;
  }

  // RViz 的 2D Nav Goal 在这里仅作为统一触发入口，不保留其原始单点导航目标。
  move_base_client_.cancelAllGoals();
  executeCoveragePath(path_indices);
  is_executing_ = false;
}

void BcdPlanner::buildObstacleLookup() {
  obstacle_grid_.assign(static_cast<std::size_t>(map_width_ * map_height_), 0U);
  if (!latest_map_ || resolution_ <= 0.0) {
    return;
  }

  precomputeInflationOffsets();

  for (std::size_t linear_index = 0; linear_index < latest_map_->data.size(); ++linear_index) {
    const int occupancy = latest_map_->data[linear_index];
    // 与 A* / Dijkstra 保持一致：未知区与占用概率 >= 50 的栅格都视为障碍。
    if (occupancy < 0 || occupancy >= 50) {
      const int obstacle_x = static_cast<int>(linear_index % static_cast<std::size_t>(map_width_));
      const int obstacle_y = static_cast<int>(linear_index / static_cast<std::size_t>(map_width_));

      for (const std::pair<int, int>& offset : inflation_offsets_) {
        const int inflated_x = obstacle_x + offset.first;
        const int inflated_y = obstacle_y + offset.second;

        if (inBounds(inflated_x, inflated_y)) {
          obstacle_grid_[static_cast<std::size_t>(toIndex(inflated_x, inflated_y))] = 1U;
        }
      }
    }
  }
}

void BcdPlanner::precomputeInflationOffsets() {
  inflation_offsets_.clear();
  const int inflation_radius_in_cells = static_cast<int>(std::ceil(robot_radius_ / resolution_));

  for (int offset_y = -inflation_radius_in_cells; offset_y <= inflation_radius_in_cells; ++offset_y) {
    for (int offset_x = -inflation_radius_in_cells; offset_x <= inflation_radius_in_cells; ++offset_x) {
      // 关键步骤：仍使用欧式距离做圆形膨胀，保证与前面最优路径算法的碰撞边界一致。
      if (std::hypot(static_cast<double>(offset_x), static_cast<double>(offset_y)) * resolution_ <=
          robot_radius_) {
        inflation_offsets_.emplace_back(offset_x, offset_y);
      }
    }
  }
}

bool BcdPlanner::lookupStartPose(double& start_world_x, double& start_world_y) {
  tf::StampedTransform transform;

  try {
    tf_listener_.waitForTransform("map", "base_footprint", ros::Time(0), ros::Duration(0.2));
    tf_listener_.lookupTransform("map", "base_footprint", ros::Time(0), transform);
  } catch (tf::TransformException& ex) {
    ROS_WARN_STREAM("BCD C++: 获取 map -> base_footprint 失败，停止规划。" << ex.what());
    return false;
  }

  start_world_x = transform.getOrigin().x();
  start_world_y = transform.getOrigin().y();
  return true;
}

bool BcdPlanner::waitForMoveBaseServer() {
  while (ros::ok()) {
    if (move_base_client_.waitForServer(ros::Duration(2.0))) {
      return true;
    }

    ROS_WARN("BCD C++: 等待 /move_base Action Server 启动。");
  }

  return false;
}

bool BcdPlanner::inBounds(int grid_x, int grid_y) const {
  return grid_x >= 0 && grid_x < map_width_ && grid_y >= 0 && grid_y < map_height_;
}

bool BcdPlanner::isObstacle(int grid_x, int grid_y) const {
  if (!inBounds(grid_x, grid_y)) {
    return true;
  }

  return obstacle_grid_[static_cast<std::size_t>(toIndex(grid_x, grid_y))] != 0U;
}

int BcdPlanner::toIndex(int grid_x, int grid_y) const {
  return grid_y * map_width_ + grid_x;
}

std::pair<int, int> BcdPlanner::indexToGrid(int linear_index) const {
  return std::make_pair(linear_index % map_width_, linear_index / map_width_);
}

std::pair<int, int> BcdPlanner::worldToGrid(double world_x, double world_y) const {
  return std::make_pair(static_cast<int>((world_x - origin_x_) / resolution_),
                        static_cast<int>((world_y - origin_y_) / resolution_));
}

std::pair<double, double> BcdPlanner::gridToWorld(int grid_x, int grid_y) const {
  return std::make_pair(origin_x_ + (static_cast<double>(grid_x) + 0.5) * resolution_,
                        origin_y_ + (static_cast<double>(grid_y) + 0.5) * resolution_);
}

std::vector<int> BcdPlanner::sampledRows(int sweep_step_cells) const {
  std::vector<int> rows;

  for (int row_y = 0; row_y < map_height_; row_y += sweep_step_cells) {
    rows.push_back(row_y);
  }

  if (rows.empty() || rows.back() != map_height_ - 1) {
    rows.push_back(map_height_ - 1);
  }

  return rows;
}

std::vector<BcdPlanner::Segment> BcdPlanner::extractFreeSegments(int row_y) const {
  std::vector<Segment> segments;
  int grid_x = 0;

  while (grid_x < map_width_) {
    while (grid_x < map_width_ && isObstacle(grid_x, row_y)) {
      ++grid_x;
    }

    if (grid_x >= map_width_) {
      break;
    }

    const int segment_start = grid_x;
    while (grid_x + 1 < map_width_ && !isObstacle(grid_x + 1, row_y)) {
      ++grid_x;
    }

    segments.push_back(Segment{row_y, segment_start, grid_x, -1});
    ++grid_x;
  }

  return segments;
}

bool BcdPlanner::segmentsOverlap(const Segment& lhs, const Segment& rhs) const {
  return lhs.x_start <= rhs.x_end && rhs.x_start <= lhs.x_end;
}

std::vector<int> BcdPlanner::traceScanSegment(const Segment& segment, bool left_to_right) const {
  std::vector<int> traced_indices;
  const int start_x = left_to_right ? segment.x_start : segment.x_end;
  const int end_x = left_to_right ? segment.x_end : segment.x_start;
  const int step_x = end_x >= start_x ? 1 : -1;
  int grid_x = start_x;

  while (true) {
    // 关键步骤：牛耕扫描段沿着整条线逐栅格检查，一旦撞到障碍立即截断，不允许穿墙。
    if (isObstacle(grid_x, segment.y)) {
      break;
    }

    traced_indices.push_back(toIndex(grid_x, segment.y));
    if (grid_x == end_x) {
      break;
    }

    grid_x += step_x;
  }

  return traced_indices;
}

std::vector<int> BcdPlanner::planSafePath(int start_x, int start_y, int goal_x, int goal_y) const {
  if (!inBounds(start_x, start_y) || !inBounds(goal_x, goal_y)) {
    return std::vector<int>();
  }

  const int start_index = toIndex(start_x, start_y);
  const int goal_index = toIndex(goal_x, goal_y);
  if (isObstacle(start_x, start_y) || isObstacle(goal_x, goal_y)) {
    return std::vector<int>();
  }

  if (start_index == goal_index) {
    return std::vector<int>(1, start_index);
  }

  static const std::vector<std::pair<int, int>> kMotionModel = {
      std::make_pair(1, 0), std::make_pair(0, 1), std::make_pair(-1, 0), std::make_pair(0, -1)};

  const double start_h = heuristic(start_x, start_y, goal_x, goal_y);
  std::priority_queue<OpenItem> open_set;
  std::unordered_map<int, GridNode> node_lookup;
  std::unordered_set<int> closed_set;
  node_lookup.reserve(static_cast<std::size_t>(map_width_ * map_height_ / 4));
  closed_set.reserve(static_cast<std::size_t>(map_width_ * map_height_ / 4));

  node_lookup.emplace(start_index, GridNode{start_x, start_y, 0.0, start_h, -1});
  open_set.push(OpenItem{start_h, start_h, start_index});

  while (!open_set.empty()) {
    const OpenItem current_item = open_set.top();
    open_set.pop();

    if (closed_set.find(current_item.index) != closed_set.end()) {
      continue;
    }

    const auto current_it = node_lookup.find(current_item.index);
    if (current_it == node_lookup.end()) {
      continue;
    }

    const GridNode& current_node = current_it->second;
    if (current_item.index == goal_index) {
      return reconstructPath(goal_index, node_lookup);
    }

    closed_set.insert(current_item.index);

    for (const std::pair<int, int>& motion : kMotionModel) {
      const int next_x = current_node.x + motion.first;
      const int next_y = current_node.y + motion.second;

      if (!inBounds(next_x, next_y) || isObstacle(next_x, next_y)) {
        continue;
      }

      const int next_index = toIndex(next_x, next_y);
      if (closed_set.find(next_index) != closed_set.end()) {
        continue;
      }

      const double tentative_g = current_node.g + 1.0;
      const double heuristic_cost = heuristic(next_x, next_y, goal_x, goal_y);

      const auto existing_it = node_lookup.find(next_index);
      if (existing_it != node_lookup.end() && tentative_g >= existing_it->second.g) {
        continue;
      }

      node_lookup[next_index] = GridNode{next_x, next_y, tentative_g, heuristic_cost, current_item.index};
      open_set.push(OpenItem{tentative_g + heuristic_cost, heuristic_cost, next_index});
    }
  }

  return std::vector<int>();
}

std::vector<int> BcdPlanner::reconstructPath(
    int goal_index, const std::unordered_map<int, GridNode>& node_lookup) const {
  std::vector<int> path_indices;
  int current_index = goal_index;

  while (current_index != -1) {
    path_indices.push_back(current_index);
    const auto node_it = node_lookup.find(current_index);
    if (node_it == node_lookup.end()) {
      break;
    }

    current_index = node_it->second.parent_index;
  }

  std::reverse(path_indices.begin(), path_indices.end());
  return path_indices;
}

void BcdPlanner::appendIndices(const std::vector<int>& extension, std::vector<int>& path_indices) const {
  for (const int linear_index : extension) {
    if (path_indices.empty() || path_indices.back() != linear_index) {
      path_indices.push_back(linear_index);
    }
  }
}

std::vector<int> BcdPlanner::compressPath(const std::vector<int>& path_indices) const {
  if (path_indices.size() < 3U) {
    return path_indices;
  }

  auto sign = [](int value) {
    if (value > 0) {
      return 1;
    }

    if (value < 0) {
      return -1;
    }

    return 0;
  };

  std::vector<int> compressed_path;
  compressed_path.reserve(path_indices.size());
  compressed_path.push_back(path_indices.front());

  bool has_previous_direction = false;
  std::pair<int, int> previous_direction = std::make_pair(0, 0);

  for (std::size_t waypoint_index = 1; waypoint_index < path_indices.size(); ++waypoint_index) {
    const std::pair<int, int> previous_grid = indexToGrid(path_indices[waypoint_index - 1U]);
    const std::pair<int, int> current_grid = indexToGrid(path_indices[waypoint_index]);
    const std::pair<int, int> current_direction =
        std::make_pair(sign(current_grid.first - previous_grid.first),
                       sign(current_grid.second - previous_grid.second));

    if (!has_previous_direction) {
      previous_direction = current_direction;
      has_previous_direction = true;
      continue;
    }

    if (current_direction != previous_direction) {
      if (compressed_path.back() != path_indices[waypoint_index - 1U]) {
        compressed_path.push_back(path_indices[waypoint_index - 1U]);
      }

      previous_direction = current_direction;
    }
  }

  if (compressed_path.back() != path_indices.back()) {
    compressed_path.push_back(path_indices.back());
  }

  return compressed_path;
}

std::vector<int> BcdPlanner::generateCellPath(const std::vector<Segment>& cell_segments) const {
  std::vector<int> dense_path;
  bool left_to_right = true;

  for (const Segment& segment : cell_segments) {
    const std::vector<int> segment_path = traceScanSegment(segment, left_to_right);
    left_to_right = !left_to_right;

    if (segment_path.empty()) {
      continue;
    }

    if (!dense_path.empty()) {
      const std::pair<int, int> current = indexToGrid(dense_path.back());
      const std::pair<int, int> segment_start = indexToGrid(segment_path.front());
      const std::vector<int> connector =
          planSafePath(current.first, current.second, segment_start.first, segment_start.second);

      if (connector.empty()) {
        ROS_WARN("BCD C++: Cell 内部扫描段之间无法安全连接，终止当前 Cell。");
        break;
      }

      appendIndices(connector, dense_path);
    }

    appendIndices(segment_path, dense_path);
  }

  return dense_path;
}

std::vector<int> BcdPlanner::buildCoveragePath(int start_x, int start_y) const {
  if (!latest_map_ || resolution_ <= 0.0) {
    return std::vector<int>();
  }

  // 0.2m 是相邻扫掠线的目标间距，统一换算为栅格步长后再做牛耕式路径采样。
  const int sweep_step_cells = std::max(1, static_cast<int>(std::round(sweep_spacing_ / resolution_)));
  const std::vector<int> rows = sampledRows(sweep_step_cells);

  std::vector<Segment> all_segments;
  std::unordered_map<int, std::vector<int>> cell_segments;
  std::vector<int> previous_row_segment_ids;
  int next_cell_id = 0;

  for (const int row_y : rows) {
    const std::vector<Segment> extracted_segments = extractFreeSegments(row_y);
    std::vector<int> current_row_segment_ids;

    for (Segment segment : extracted_segments) {
      std::unordered_set<int> parent_cell_ids;
      for (const int previous_segment_id : previous_row_segment_ids) {
        const Segment& previous_segment = all_segments[static_cast<std::size_t>(previous_segment_id)];
        if (segmentsOverlap(previous_segment, segment)) {
          parent_cell_ids.insert(previous_segment.cell_id);
        }
      }

      // 简化版 BCD：若扫掠线拓扑发生分裂或合并，则开启新 Cell；否则沿用唯一父 Cell。
      if (parent_cell_ids.size() == 1U) {
        segment.cell_id = *parent_cell_ids.begin();
      } else {
        segment.cell_id = next_cell_id++;
      }

      all_segments.push_back(segment);
      const int segment_id = static_cast<int>(all_segments.size() - 1U);
      cell_segments[segment.cell_id].push_back(segment_id);
      current_row_segment_ids.push_back(segment_id);
    }

    previous_row_segment_ids = current_row_segment_ids;
  }

  std::unordered_map<int, std::vector<int>> cell_paths;
  for (const auto& item : cell_segments) {
    std::vector<Segment> ordered_segments;
    ordered_segments.reserve(item.second.size());

    for (const int segment_id : item.second) {
      ordered_segments.push_back(all_segments[static_cast<std::size_t>(segment_id)]);
    }

    std::sort(ordered_segments.begin(), ordered_segments.end(),
              [](const Segment& lhs, const Segment& rhs) {
                if (lhs.y != rhs.y) {
                  return lhs.y < rhs.y;
                }
                return lhs.x_start < rhs.x_start;
              });

    const std::vector<int> cell_path = generateCellPath(ordered_segments);
    if (!cell_path.empty()) {
      cell_paths[item.first] = cell_path;
    }
  }

  struct Candidate {
    double distance;
    int cell_id;
    bool reverse_cell;
  };

  std::vector<int> dense_path;
  std::pair<int, int> current = std::make_pair(start_x, start_y);

  while (!cell_paths.empty()) {
    std::vector<Candidate> candidates;
    candidates.reserve(cell_paths.size() * 2U);

    for (const auto& item : cell_paths) {
      const std::vector<int>& cell_path = item.second;
      const std::pair<int, int> entry = indexToGrid(cell_path.front());
      const std::pair<int, int> exit = indexToGrid(cell_path.back());

      candidates.push_back(Candidate{std::hypot(static_cast<double>(entry.first - current.first),
                                                static_cast<double>(entry.second - current.second)),
                                     item.first, false});
      candidates.push_back(Candidate{std::hypot(static_cast<double>(exit.first - current.first),
                                                static_cast<double>(exit.second - current.second)),
                                     item.first, true});
    }

    std::sort(candidates.begin(), candidates.end(),
              [](const Candidate& lhs, const Candidate& rhs) { return lhs.distance < rhs.distance; });

    bool selected = false;
    int selected_cell_id = -1;
    std::vector<int> selected_connector;
    std::vector<int> selected_cell_path;

    for (const Candidate& candidate : candidates) {
      const std::vector<int>& cell_path = cell_paths.at(candidate.cell_id);
      std::vector<int> ordered_cell_path = cell_path;
      if (candidate.reverse_cell) {
        std::reverse(ordered_cell_path.begin(), ordered_cell_path.end());
      }

      const std::pair<int, int> entry = indexToGrid(ordered_cell_path.front());
      const std::vector<int> connector =
          planSafePath(current.first, current.second, entry.first, entry.second);
      if (connector.empty()) {
        continue;
      }

      selected = true;
      selected_cell_id = candidate.cell_id;
      selected_connector = connector;
      selected_cell_path = ordered_cell_path;
      break;
    }

    if (!selected) {
      ROS_WARN("BCD C++: 剩余 Cell 无法与当前路径安全连接，提前结束覆盖。");
      break;
    }

    appendIndices(selected_connector, dense_path);
    appendIndices(selected_cell_path, dense_path);
    cell_paths.erase(selected_cell_id);
    current = indexToGrid(dense_path.back());
  }

  return compressPath(dense_path);
}

double BcdPlanner::heuristic(int grid_x, int grid_y, int goal_x, int goal_y) const {
  // 关键步骤：安全连接统一使用曼哈顿启发式，匹配 4 邻域搜索并避免不必要的斜穿。
  return std::abs(goal_x - grid_x) + std::abs(goal_y - grid_y);
}

double BcdPlanner::computeWaypointYaw(const std::vector<int>& path_indices,
                                      std::size_t waypoint_index) const {
  if (path_indices.size() < 2U) {
    return 0.0;
  }

  int from_index = path_indices[waypoint_index];
  int to_index = path_indices[waypoint_index];

  if (waypoint_index + 1U < path_indices.size()) {
    to_index = path_indices[waypoint_index + 1U];
  } else {
    from_index = path_indices[waypoint_index - 1U];
  }

  const std::pair<int, int> from_grid = indexToGrid(from_index);
  const std::pair<int, int> to_grid = indexToGrid(to_index);
  const std::pair<double, double> from_world = gridToWorld(from_grid.first, from_grid.second);
  const std::pair<double, double> to_world = gridToWorld(to_grid.first, to_grid.second);
  return std::atan2(to_world.second - from_world.second, to_world.first - from_world.first);
}

void BcdPlanner::publishPath(const std::vector<int>& path_indices) const {
  nav_msgs::Path path_msg;
  path_msg.header.stamp = ros::Time::now();
  path_msg.header.frame_id = "map";

  path_msg.poses.reserve(path_indices.size());
  for (std::size_t waypoint_index = 0; waypoint_index < path_indices.size(); ++waypoint_index) {
    const std::pair<int, int> grid = indexToGrid(path_indices[waypoint_index]);
    // 关键步骤：发布可视化路径时仍取栅格中心点，保持与 A* / Dijkstra 一致的索引还原逻辑。
    const std::pair<double, double> world = gridToWorld(grid.first, grid.second);

    geometry_msgs::PoseStamped pose;
    pose.header = path_msg.header;
    pose.pose.position.x = world.first;
    pose.pose.position.y = world.second;
    pose.pose.orientation = tf::createQuaternionMsgFromYaw(computeWaypointYaw(path_indices, waypoint_index));
    path_msg.poses.push_back(pose);
  }

  path_pub_.publish(path_msg);
}

void BcdPlanner::executeCoveragePath(const std::vector<int>& path_indices) {
  for (std::size_t waypoint_index = 0; waypoint_index < path_indices.size() && ros::ok(); ++waypoint_index) {
    const std::pair<int, int> grid = indexToGrid(path_indices[waypoint_index]);
    const std::pair<double, double> world = gridToWorld(grid.first, grid.second);

    move_base_msgs::MoveBaseGoal goal;
    goal.target_pose.header.stamp = ros::Time::now();
    goal.target_pose.header.frame_id = "map";
    goal.target_pose.pose.position.x = world.first;
    goal.target_pose.pose.position.y = world.second;
    goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(
        computeWaypointYaw(path_indices, waypoint_index));

    move_base_client_.sendGoal(goal);
    const bool finished = move_base_client_.waitForResult(ros::Duration(goal_timeout_));
    if (!finished) {
      ROS_WARN_STREAM("BCD C++: 第 " << waypoint_index << " 个覆盖点等待超时，跳过该点。");
      move_base_client_.cancelGoal();
      continue;
    }

    const actionlib::SimpleClientGoalState state = move_base_client_.getState();
    if (state != actionlib::SimpleClientGoalState::SUCCEEDED) {
      ROS_WARN_STREAM("BCD C++: 第 " << waypoint_index << " 个覆盖点执行失败，状态为 "
                                     << state.toString() << "，跳过该点。");
      continue;
    }
  }
}

}  // namespace coverage
}  // namespace mr_traditional_planner

int main(int argc, char** argv) {
  ros::init(argc, argv, "bcd_planner_cpp");
  mr_traditional_planner::coverage::BcdPlanner planner;
  ros::spin();
  return 0;
}
