...
void DijkstraPlanner::buildObstacleLookup() {
...
void DijkstraPlanner::precomputeInflationOffsets() {
...
bool DijkstraPlanner::isObstacle(int grid_x, int grid_y) const {
...
int DijkstraPlanner::toIndex(int grid_x, int grid_y) const {
  return grid_y * map_width_ + grid_x;
}

std::pair<int, int> DijkstraPlanner::worldToGrid(double world_x, double world_y) const {
  return std::make_pair(static_cast<int>((world_x - origin_x_) / resolution_),
                        static_cast<int>((world_y - origin_y_) / resolution_));
}

std::pair<double, double> DijkstraPlanner::gridToWorld(int grid_x, int grid_y) const {
...
std::vector<int> DijkstraPlanner::planPath(int start_x, int start_y, int goal_x, int goal_y) {
  ...
  node_lookup.emplace(start_index, Node{start_x, start_y, 0.0, 0.0, -1});
  open_set.push(OpenItem{0.0, start_index});
  ...
  while (!open_set.empty()) {
    ...
    for (const auto& motion : kMotionModel) {
      ...
      node_lookup[next_index] = Node{next_x, next_y, tentative_g, 0.0, current_item.index};
      open_set.push(OpenItem{tentative_g, next_index});
    }
  }
...
std::vector<int> DijkstraPlanner::reconstructPath(
    int goal_index, const std::unordered_map<int, Node>& node_lookup) const {
